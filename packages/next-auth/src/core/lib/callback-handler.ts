...
export default async function callbackHandler(params: {
	...
	profile: User | AdapterUser | { email: string } | { phoneNumber: string }
	...
	if (!["email", "oauth", "sms"].includes(account.type))
	...
	if (account.type === "email") {
		...
		// If signing in with an phoneNumber, check if an account with the same phoneNumber address exists already
		...
		if (userByPhoneNumber) {
			...
			// Update emailVerified property on the user object
			user = await updateUser({ id: userByPhoneNumber.id, smsVerified: new Date() })
			await events.updateUser?.({ user })
		} else {
			const { id: _, ...newUser } = { ...profile, smsVerified: new Date() }
			// Create user account if there isn't one for the phoneNumber address already
			user = await createUser(newUser)
			await events.createUser?.({ user })
			isNewUser = true
		}
		...
		if (userByAccount) {
			...
			if (userByEmail) {
				...
				const userByPhoneNumber = profile.phoneNumber
				? await getUserByPhoneNumber(profile.phoneNumber)
				: null
				if( userByPhoneNumber ){
					const provider = options.provider as OAuthConfig<any>
					if (provider?.allowDangerousPhoneNumberAccountLinking) {
						// If you trust the oauth provider to correctly verify email addresses, you can opt-in to
						// account linking even when the user is not signed-in.
						...
						// to sign in via email to verify their identity and then link the accounts.
						throw new AccountNotLinkedError(
							"Another account already exists with the same phone number address"
						)
					}
				} else {
