...
export default async function callback(params: {
	...
	if (provider.type === "oauth") {
		...
		try {
			...
			const profile = await getAdapterUserFromPhoneNumber({
			...
			const account = {
				providerAccountId: profile.phoneNumber,
				type: "sms" as const,
				provider: provider.id,
			}
			...
			if ((error as Error).name === "CreateUserError") {
				return { redirect: `${url}/error?error=SMSCreateAccount`, cookies }
			}
			logger.error("CALLBACK_SMS_ERROR", error as Error)
			return { redirect: `${url}/error?error=Callback`, cookies }
		}
	}
